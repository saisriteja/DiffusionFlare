...
class Uformer(nn.Module):
    ...
    def forward(self, rgb, depth, mask=None):
        ...
        #Encoder
        rgb_conv0 = self.encoderlayer_0(y_rgb,mask=mask)
        rgb_pool0 = self.dowsample_0(rgb_conv0)

        depth_conv0 = self.encoderlayer_0(y_depth,mask=mask)
        ...
        frm0 = FRM(dim=embed_dims[0], reduction=1)
        ffm0 = FFM(dim=embed_dims[0], reduction=1, num_heads=num_heads[0], norm_layer=norm_fuse)

        pool0 = pool0.transpose(1,2).reshape(-1,embed_dims[0],self.reso//2,self.reso//2)
        # pool0 = pool0.reshape(pool0.shape[0],pool0.shape[2],  int(pool0.shape[1] ** 0.5), int(pool0.shape[1] ** 0.5))
        info['bf_ffm_pool0'] = pool0


        pool0_ffm, pool0_ffm = frm0(pool0, pool0)
        info['pool0'] = pool0_ffm
        out = ffm0(pool0_ffm, pool0_ffm)
        info['out0'] = out

        pool0 = out.reshape(-1,embed_dims[0],self.reso//2 * self.reso//2).transpose(1,2)
...
class Uformer_CatCross(nn.Module):
    def __init__(self, img_size=128, in_chans=3,out_chans=3,
        ...
        # Decoder
        self.upsample_0 = upsample(embed_dim*16, embed_dim*8)
        self.decoderlayer_encoderlayer_00 = CatCrossUformerLayer(dim=embed_dim*8,
                            output_dim=embed_dim*8,
                            input_resolution=(img_size // (2 ** 3),
                                                img_size // (2 ** 3)),
                            depth=depths[5],

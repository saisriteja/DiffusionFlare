...
@DATASET_REGISTRY.register()
class Depth_Image_Pair_Loader(data.Dataset):
    def __init__(self, opt):
        super(Depth_Image_Pair_Loader, self).__init__()
        self.pipe = pipeline(task="depth-estimation", model="depth-anything/Depth-Anything-V2-Small-hf", device=torch.distributed.get_rank())
        self.opt = opt
        self.gt_folder, self.lq_folder = opt['dataroot_gt'], opt['dataroot_lq']
        self.paths = glod_from_folder([self.lq_folder, self.gt_folder], ['lq', 'gt'])
        self.to_tensor=transforms.ToTensor()
        self.gt_size=opt['gt_size']
        self.transform = transforms.Compose([transforms.Resize(self.gt_size), transforms.CenterCrop(self.gt_size)])
    ...
    def __getitem__(self, index):
        ...
        lq_path = self.paths['lq'][index]
        img_lq=self.transform(Image.open(lq_path).convert('RGB'))
        img_gt=self.tensor_transform(self.transform(Image.open(gt_path).convert('RGB')))
        img_depth = self.pipe(img_lq)["depth"]
        img_lq=self.tensor_transform(img_lq)
        img_depth = self.tensor_transform(img_depth)
        return {'lq': img_lq, 'gt': img_gt, 'depth': img_depth}

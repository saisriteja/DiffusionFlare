...
# Uformer Class with CMX (currently with 1 enc & dec)
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        # Encoder
        self.encoderlayer_rgb = BasicUformerLayer(dim=embed_dim,
        ...
        self.encoderlayer_depth = BasicUformerLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[0],
        ...
        self.decoderlayer = BasicUformerLayer(dim=embed_dim*2,
        ...
        self.downsample = Downsample(embed_dim, embed_dim*2)
        self.upsample = Upsample(embed_dim*2, embed_dim)
        self.upsample_bottleneck = Upsample(embed_dim*2, embed_dim)
        self.bottleneck = nn.Sequential(
            InputProj(in_channel=embed_dim*2, out_channel=embed_dim*2, kernel_size=3, stride=1, act_layer=nn.LeakyReLU),
            BasicUformerLayer(dim=embed_dim*2, output_dim=embed_dim*2,
    ...
    def forward(self, rgb, depth, mask=None):
        ...
        # Encoder
        rgb_enc = self.encoderlayer_rgb(y_rgb, mask=mask)
        depth_enc = self.encoderlayer_depth(y_depth, mask=mask)

        # Downsample both
        rgb_down = self.downsample(rgb_enc)
        depth_down = self.downsample(depth_enc)

        rgb_down = rgb_down.reshape(B, self.embed_dim * 2, H // 2, W // 2).permute(0, 1, 2, 3).contiguous()
        depth_down = depth_down.reshape(B, self.embed_dim * 2, H // 2, W // 2).permute(0, 1, 2, 3).contiguous()

        x_rgb, x_e = self.FRMs[0](rgb_down, depth_down)
        x_out = self.FFMs[0](x_rgb, x_e)

        # BottleNeck
        x = self.bottleneck(x_out)

        # Decoder
        x = self.upsample_bottleneck(x)

        B, C, H, W = x_out.shape
        x_out = x_out.reshape(B, C, H * W).permute(0, 2, 1).contiguous()
        x_out = self.upsample(x_out)
        deconv = torch.cat([x, x_out], -1)
        deconv = self.decoderlayer(deconv, mask=mask)

        y = self.output_proj(deconv)
        return y
...
if __name__ == "__main__":
    ...
    rgb_input = torch.randn(1, 3, input_size, input_size)
    depth_input = torch.randn(1, 3, input_size, input_size)

    op = model_restoration(rgb_input,depth_input)
    print(op.shape)

    # flops = FlopCountAnalysis(model_restoration, (rgb_input,depth_input))
